...
import { Load,SearchResult } from './load.model';
...
function matches(tables: Load, term: string, pipe: PipeTransform) {
    return  pipe.transform(tables.status).includes(term)
        || tables.loadNumber.toLowerCase().includes(term)
        || tables.customer.toLowerCase().includes(term)
        || tables.carrier.toLowerCase().includes(term)
        || tables.pickup.toLowerCase().includes(term)
        || tables.pickupDate.toLowerCase().includes(term)
        || tables.delivery.toLowerCase().includes(term)
...
export class LoadService {
    ...
    // tslint:disable-next-line: variable-name
    private _tables$ = new BehaviorSubject<Load[]>([]);
    // tslint:disable-next-line: variable-name
    ...
    constructor(private pipe: DecimalPipe) {
        this._search$.pipe(
            ...
            this._tables$.next(result.loads);
    ...
    private _search(): Observable<SearchResult> {
        ...
        // 1. sort
        let loads = sort(Loads, sortColumn, sortDirection);
        ...
        // 3. paginate
        this.totalRecords = loads.length;
